...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    #Using l2_regularizer(1e-3) in all layers to penalize large weights.
    #1x1 convolution(of 7th layer) to maintain spatial information
    ...
    #Upsampling the input to the original image size
    output_1 = tf.layers.conv2d_transpose(conv_1x1_7, num_classes, 4, strides= (2, 2), padding='same',
    									kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3), kernel_initializer=tf.truncated_normal_initializer(stddev=0.01))

    #1x1 convolution(of 4th layer) to maintain spatial information
    conv_1x1_4 = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, padding='same',
    							kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3), kernel_initializer=tf.truncated_normal_initializer(stddev=0.01))

    #Adding Skip connection to retain information, to maintain the big picture without losing data during different convolution layers.
    skip_1 = tf.add(output_1, conv_1x1_4)

    #Upsampling
    output_2 = tf.layers.conv2d_transpose(skip_1, num_classes, 4, strides= (2, 2), padding='same',
    									kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3), kernel_initializer=tf.truncated_normal_initializer(stddev=0.01))

    #1x1 convolution(of 3rd layer) to maintain spatial information
    conv_1x1_3 = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, padding='same',
    							kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3), kernel_initializer=tf.truncated_normal_initializer(stddev=0.01))

    #Adding Skip connection to retain information, to maintain the big picture without losing data during different convolution layers.
    skip_2 = tf.add(output_2, conv_1x1_3)

    #Upsampling x 8
    output_3 = tf.layers.conv2d_transpose(skip_2, num_classes, 16, strides= (8, 8), padding='same',
    									kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3), kernel_initializer=tf.truncated_normal_initializer(stddev=0.01))
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    cross_entropy_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits= logits, labels= label_reshape))
    #reg_losses = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)
    #cross_entropy_loss = cross_entropy_loss + sum(reg_losses)

    #Define Adam optimizer with variable learning rate
    ...
    for epoch in range(epochs):
        ...
        for image, label in get_batches_fn(batch_size):
            _, loss = sess.run([train_op, cross_entropy_loss], feed_dict={input_image: image, correct_label: label,
                                    keep_prob: 0.7, learning_rate: 0.001})
            log.append('{:3f}'.format(loss))
        print(log)
        print()
...
def run():
    ...
    with tf.Session() as sess:
        ...
        logits, training_operation, cross_entropy_loss = optimize(output, correct_label, learning_rate, num_classes)        
